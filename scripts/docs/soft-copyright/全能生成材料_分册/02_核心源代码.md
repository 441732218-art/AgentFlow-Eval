...
**说明：** 下列代码摘自本软件核心业务模块（用户鉴权、租户隔离、领域模型、规则指标、混合评分、工具沙箱、异步评测编排、任务 API），已去除第三方库源码、编译产物与敏感配置；密钥与地址统一使用占位符 `your_api_key_here`、`https://api.your-llm-endpoint.example/v1`。版权年份为 **2026**，与申请表开发完成日期（2026年7月14日）所属年份一致。术语与说明书统一：评测任务（Task）、测试用例（TestSuite）、执行轨迹（Trace）、指标分（MetricScore）。

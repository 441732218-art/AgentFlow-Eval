...
| Workflow | Trigger |
|----------|---------|
| `.github/workflows/test.yml` | PR/push �?ruff, bandit (medium+), pytest cov�?0% |
| `.github/workflows/docker.yml` | **唯一**镜像流水线：PR 仅构建；main/tags 构建+推�?GHCR+Trivy |
| `.github/workflows/release.yml` | tags `v1.*` �?测试门禁 + GitHub Release Notes�?*不再**重复构建镜像�?|
| `.github/workflows/build-windows.yml` / `desktop-macos.yml` | 桌面安装�?�?Release Assets |
| `.github/workflows/deploy.yml` | manual SSH deploy (secrets) |

> 已删除冗余的 `build.yml`，避免与 `docker.yml` 双推镜像导致分钟浪费与偶发失败�?
---

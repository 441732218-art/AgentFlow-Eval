...
Runtime execution path** via `RuntimeService` — without modifying frozen
...
Application providers (applications/bootstrap.py)
        |
        v
runtime/service/tooling_bootstrap.py   ← wiring boundary (may import applications)
        |
        ...
        |
        v
RuntimeService → AgentExecutor → ExecutionPipeline → ToolExecutionEngine
```
...
**Application layer** — explicit, no hidden globals:

```python
bootstrap_applications(registry, handler_registry)
```

**Production wiring** — idempotent, service-layer only:
...
Chain verified: Registry → Engine → LocalAdapter → Handler → Pipeline output.
...
- Real business providers: add under `applications/<name>_provider/`, register in
  `DEFAULT_APPLICATION_PROVIDERS` — no Runtime Core changes

...
**Phase:** 8.4 — ToolExecutionEngine → ExecutionPipeline Integration  
**Date:** 2026-08-31
...
ExecutionPipeline       (Phase 8.4 primary change)
       |
       +-- before_hooks (TraceHook, MemoryHook — unchanged order)
       +-- execute_step
       |      |
       |      +-- no tool → default stub output
       |      +-- tool_definition present → ToolExecutionEngine.execute()
       +-- after_hooks (unchanged order)
       |
       v
ToolExecutionEngine     (frozen since Phase 8.2–8.3)
       |
       +----------------+
       |                |
...
after_hooks
    ↓
return result → AgentExecutor → ExecutionResult
```
...
Next: Tool Registry lifecycle — register definitions + handlers at service bootstrap, resolve tool by name in pipeline.

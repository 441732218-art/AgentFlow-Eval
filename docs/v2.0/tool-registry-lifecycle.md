...
1. `bootstrap.py` defines `DEFAULT_LOCAL_HANDLERS` — a `dict` of tool name →
   **function reference** (e.g. `example_echo_handler`).
...
callables registered at startup — the same static model as tool definitions.

Missing local handler → `MissingLocalHandlerError` (unchanged, adapter layer).

Missing tool definition → `ToolNotFoundError` (registry layer, Phase 8.5).
...
```
get_tool_registry()
  → registry.get("example.remote_search")
  → ToolExecutionEngine.execute(definition, args)
  → RemoteToolExecutorAdapter (executor_type="remote")
  → InMemoryRemoteClient (test mock; simulates external provider)
  → ToolProviderResponse → result.output
```

### Conclusion: future Application Layer integration

**Yes — with configuration-only changes at the Application boundary.**
...
| Marketplace / dynamic tools | Future phase | Requires policy, auth, and unregister semantics — out of scope for 8.5 |

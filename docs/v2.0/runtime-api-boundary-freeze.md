...
**Phase:** 7.1.8 â€?API Boundary Freeze Review  
**Date:** 2026-08-31  
...
**Answer:** **YES â€?with documented guardrails.** The in-process stack (`RuntimeService` â†?`AgentExecutor` â†?`ExecutionPipeline` â†?Hooks â†?`ExecutionStore`) is stable (45 unit tests passing). HTTP exposure has not started; Phase 7.2 must implement adapters that honor the frozen boundaries below. Legacy `app.core.runtime` HTTP routes coexist and must not be conflated with new routes.
...
AgentExecutor.execute()
        |
        v
ExecutionPipeline â†?Hooks â†?ExecutionResult
        |
        v
ExecutionStore.save(ExecutionRecord)
        |
        v
ExecutionResponseDTO  â†? HTTP response
```
...
POST /api/v1/runtime/agents/{id}/run
        |
        v
app.core.runtime.AgentRuntime.run()   â†?legacy Sprint 1 MVP
```
...
| Question | Answer |
|----------|--------|
| **Does library layer satisfy `router â†?RuntimeService â†?AgentExecutor`?** | **YES** â€?service encapsulates executor + store |
| **Is HTTP layer compliant today?** | **N/A** â€?no Phase 7.2 routes exist yet |
| **Risk if Phase 7.2 skips RuntimeService** | **HIGH** â€?would bypass persistence, DTO mapping, future cross-cutting concerns |

### 1.5 Frozen rule for Phase 7.2

```text
ALLOW:   router.handler â†?RuntimeService.execute() / get_execution()
FORBID:  router.handler â†?AgentExecutor.execute()
FORBID:  router.handler â†?ExecutionStore directly
FORBID:  router.handler â†?app.core.runtime (for new execute/query routes)
```
...
|-------|-------|---------------|
| `ExecutionResult` | Executor internal | **FORBIDDEN** â€?executor return type only |
| `ExecutionRecord` | ExecutionStore persistence | **FORBIDDEN direct** â€?map via query DTO |
| `ExecutionResponseDTO` | Service â†?API (execute) | **ALLOWED** for `POST /runtime/execute` |
...
1. **No internal model leakage** â€?DTO excludes `agent_id`, `trace_reference`, `created_at`, store internals.
2. **ExecutionStore not API-locked** â€?store can evolve (`ExecutionRecord` fields) if API uses adapter mapping, not raw record serialization.
3. **No Tool coupling** â€?DTO has zero tool name/metadata/execution fields; `RuntimeService` does not reference `ToolRegistry` in execute path.
4. **Stable minimal surface** â€?four fields sufficient for execute response; query endpoint needs separate DTO (see Â§3).
...
| ExecutionStore vs Memory | **YES** | Phase 7.1 removed `execution_id â†?MemoryProvider` writes; Memory uses `memory_key` only |
...
- `tool_calls`
- `knowledge results`
- Full `ExecutionRecord.agent_id` (unless product explicitly requires â€?default exclude)
...
but must **not** inline trace events. Future trace query endpoint (postâ€“Phase 7.2) resolves reference separately.
...
- When `ENABLE_RUNTIME_V2=False` â†?HTTP 503 `{error: "runtime_disabled"}`
- When `ENABLE_RUNTIME_V2=True` â†?legacy agent registry/run endpoints active
...
```text
ENABLE_RUNTIME_V2=false  â†? ALL /api/v1/runtime/* including new execute/query â†?503
ENABLE_RUNTIME_V2=true   â†? New + legacy runtime routes available (legacy marked deprecated)
```
...
```text
/runtime/execute              â†?RuntimeService.execute()     [NEW - app.runtime]
/runtime/executions/{id}      â†?RuntimeService.get_execution() [NEW - app.runtime]

/runtime/agents               â†?AgentRegistry               [LEGACY - app.core.runtime]
/runtime/agents/{id}/run      â†?AgentRuntime.run()          [LEGACY - app.core.runtime]
```
...
| `AgentExecutor.tool_registry` | Stored, **not invoked** | **Low** â€?constructor slot only; API must not expose registry injection over HTTP in Phase 7.2 |
| `ExecutionPipeline._execute_step` | Placeholder, no tools | **None today** |

### 6.3 Tool Contract Isolation verdict

**PASS** â€?Phase 7.2 DTO/Service layer is Tool-free. Phase 7.2 implementation must not add tool routes or tool fields to response schemas.
...
Result: 45 passed, 0 failed, 1 warning (structlog â€?non-blocking)
```
...
1. **RuntimeService** is the correct and implemented API boundary â€?executor + store encapsulated.
...
6. **45/45** runtime unit tests pass â€?core is stable.
...
1. **Add HTTP adapter routes** in `endpoints/runtime.py`: `POST /execute`, `GET /executions/{execution_id}` â†?`RuntimeService` only.
...
4. **Add API integration tests** (Phase 7.2) â€?unit tests alone do not cover HTTP wiring.
5. **OpenAPI deprecation markers** on legacy `/agents/*` routes.
6. **Do not add Tool HTTP endpoints** â€?defer to Phase 8 (Tool Provider Protocol).
...
|---|-----------------|---------------------------|
| 1 | HTTP â†?RuntimeService only | Code review: no `AgentExecutor` import in router |
...
| 5 | ENABLE_RUNTIME_V2 gates all /runtime/* | Test with flag false â†?503 |

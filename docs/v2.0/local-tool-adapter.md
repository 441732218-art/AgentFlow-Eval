...
**Phase:** 8.2.2 — Local Tool Executor Adapter  
**Date:** 2026-08-31
...
- Non-callable handler → `TypeError`
- Duplicate name → `DuplicateLocalHandlerError`
- Missing handler at execution → `MissingLocalHandlerError`
...
```text
definition.name  →  LocalHandlerRegistry.get(name)  →  handler(**arguments)
```
...
LocalHandlerRegistry["tool.name"] = tool.execute
   |
   v
LocalToolExecutorAdapter → handler(**arguments)
```
...
```text
"local" → LocalToolExecutorAdapter
```
...
| Phase | Deliverable |
|-------|-------------|
| 8.2.2 ✅ | `LocalToolExecutorAdapter` + `LocalHandlerRegistry` |

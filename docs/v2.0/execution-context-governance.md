...
```
AgentExecutor
    → ensure_execution_context(RuntimeContext)
    → ExecutionPipeline
    → ToolExecutionEngine.execute(..., context=ExecutionContext)
    → ToolExecutorAdapter.execute(..., execution_context=...)
    → RemoteToolExecutorAdapter → HttpRemoteToolClient
```

...
ToolDefinition (metadata.endpoint)
        |
RemoteToolExecutorAdapter  ← RemoteExecutionPolicy (retry / timeout guard)
        |
HttpRemoteToolClient       ← single HTTP round-trip per send()
        |
...
→ `HttpRemoteToolClient` raises `RemoteProviderError`.
...
trade.search_customer
    ...
    → HttpRemoteToolClient
    → External Trade Service HTTP endpoint
```

Requires `metadata.endpoint` on the tool definition and a production
`CredentialResolver` — not part of Phase 9.2.

---

## Related Docs

- `production-remote-tool-client.md` — Phase 8.6 initial implementation notes
- `remote-tool-provider-protocol.md` — request/response dataclasses
- `trade-application-provider.md` — Application layer template

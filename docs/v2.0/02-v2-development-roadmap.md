...
> **Strategy:** Incremental Evolution — **new modules + compatibility**, never a v1 rewrite.  
...
1. **Agent Runtime** — execute, state, (later) workflow, tool calls  
2. **Agent Evaluation** — quality scores, LLM Judge, benchmark, datasets  
3. **Observability foundation** — trace, LLM/tool, token, cost, latency  
4. **Security foundation** — keep v1 secret redaction / SSRF / RBAC; add agent/tool ACL seeds  
...
- start a **single-shot run** (query in → `AgentResult` out)
...
| Risk | Mitigation |
| --- | --- |
| Editing `openai_runner.py` “to make it cleaner” | Forbidden |
...
**1.5–2.5 weeks** (1 senior + tests). MVP = facade + one run path + tests + lite demo.
...
- Structured tool events already defined: `LogEvent.TOOL_*` — emit consistently from broker
...
| `backend/app/api/v1/endpoints/tools.py` | List/probe — additive endpoints only |
...
- Optional extra judge dimensions — via scorecard JSON already supported

### Tech

- New `core/evaluation/dataset.py` (or `core/datasets/`)
- Adapter: `Trace` / Runtime run → judge input (already `trace.steps`)
...
**2–3 weeks** for Dataset + runtime-bridge + tests. Full “panel judge / pairwise” is extra and can slip to v2.1.
...
Raise domain observability from **Intermediate → Intermediate+** (not full APM).
...
| `backend/app/models/trace.py` | Schema already has cost fields — **do not migrate**; write on new run model |
...
**1–1.5 weeks** for persist + API + reuse DAG. Full OTel = later.
...
- Secret-at-rest **plan** (vault/env pointer instead of JSON key) — implement only if it does **not** change v1 `agent_config` schema
...
- Additive `Permission` values (new enum members only — coordinate with frontend `permissions.ts`)
...
| “Fixing” redaction | Forbidden |
| Prompt guard breaking Chinese eval suites | Off by default; eval path unchanged |

### Estimated effort

**1–1.5 weeks** for ACL + optional guard. Vault = separate spike.
...
These remain **designed, not scheduled as Sprint 1–5**:
...
ReactFlow editor (frontend)
    → agentflow.graph.v1 JSON
        → core/runtime/backends/langgraph
...
backend/app/
  ...
  core/
    agent_runner/           # FROZEN v1 — wrap only
    judge_engine/           # FROZEN core — extend via new files
...
**Success:** one lite demo: create runtime agent (openai or http) → run one query → see steps → v1 Task execute still works.
